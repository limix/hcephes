...
static double P[] = {
    1.53552577301013293365E-4, 2.50888492163602060990E-3, 8.68786816565889628429E-3,
...
HCEPHES_API double hcephes_ellpe(double x) {
    ...
    if ((x <= 0.0) || (x > 1.0)) {
        if (x == 0.0)
            return 1.0;
        hcephes_mtherr("ellpe", HCEPHES_DOMAIN);
        return 0.0;
    }
    return hcephes_polevl(x, P, 10) - log(x) * (x * hcephes_polevl(x, Q, 9));
}
