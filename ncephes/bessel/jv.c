...
static double ncephes_jnt(double n, double x);
static double ncephes_jnx(double n, double x);
static double ncephes_jvs(double n, double x);
static double ncephes_recur(double *n, double x, double *newn, int cancel);
...
double ncephes_jv(double n, double x) {
    ...
    if (y == an) {
        ...
        if (n == 0.0)
            return (ncephes_j0(x));
        if (n == 1.0)
            return (sign * ncephes_j1(x));
    }

    if ((x < 0.0) && (y != an)) {
        ncephes_mtherr("Jv", DOMAIN);
        y = 0.0;
        goto done;
    }
    ...
    t = 3.6 * sqrt(an);
    if ((y < t) && (an > 21.0))
        return (sign * ncephes_jvs(n, x));
    if ((an < k) && (y > 21.0))
        return (sign * ncephes_hankel(n, x));
...
static double ncephes_jvs(double n, double x) {
    ...
    if ((ex > -1023) && (ex < 1023) && (n > 0.0) && (n < (MAXGAM - 1.0))) {
        t = pow(0.5 * x, n) / ncephes_gamma(n + 1.0);
        y *= t;
    } else {

        t = n * log(0.5 * x) - ncephes_lgam(n + 1.0);
        if (y < 0) {
            sgngam = -sgngam;
            y = -y;
        }
...
static double ncephes_jnx(double n, double x) {
    ...
    ncephes_airy(t, &ai, &aip, &bi, &bip);

    /* polynomials in expansion */
    u[0] = 1.0;
    zzi = 1.0 / zz;
    u[1] = ncephes_polevl(zzi, P1, 1) / sz;
    u[2] = ncephes_polevl(zzi, P2, 2) / zz;
    u[3] = ncephes_polevl(zzi, P3, 3) / (sz * zz);
    pp = zz * zz;
    u[4] = ncephes_polevl(zzi, P4, 4) / pp;
    u[5] = ncephes_polevl(zzi, P5, 5) / (pp * sz);
    pp *= zz;
    u[6] = ncephes_polevl(zzi, P6, 6) / pp;
    u[7] = ncephes_polevl(zzi, P7, 7) / (pp * sz);
...
static double ncephes_jnt(double n, double x) {
    ...
    F[0] = 1.0;
    F[1] = -z / 5.0;
    F[2] = ncephes_polevl(z3, PF2, 1) * zz;
    F[3] = ncephes_polevl(z3, PF3, 2);
    F[4] = ncephes_polevl(z3, PF4, 3) * z;
    G[0] = 0.3 * zz;
    G[1] = ncephes_polevl(z3, PG1, 1);
    G[2] = ncephes_polevl(z3, PG2, 2) * z;
    G[3] = ncephes_polevl(z3, PG3, 2) * zz;
