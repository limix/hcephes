...
double ncephes_hyp2f1(double a, double b, double c, double x) {
    ...
    if (fabs(ax - 1.0) < EPS) /* |x| == 1.0	*/
    {
        if (x > 0.0) {
            ...
            if (d <= 0.0)
                goto hypdiv;
            y = ncephes_gamma(c) * ncephes_gamma(d) /
                (ncephes_gamma(p) * ncephes_gamma(r));
            goto hypdon;
        }
    ...
    if (d < 0.0) {
        ...
        aid = 2 - id;
        e = c + aid;
        d2 = ncephes_hyp2f1(a, b, e, x);
        d1 = ncephes_hyp2f1(a, b, e + 1.0, x);
        q = a + b + 1.0;
        for (i = 0; i < aid; i++) {
            r = e - 1.0;
            y = (e * (r - (2.0 * e - q) * x) * d2 +
 ...
 */
hypf:
    y = pow(s, d) * ncephes_hys2f1(c - a, c - b, c, x, &err);
    goto hypdon;
...
static double ncephes_hyt2f1(double a, double b, double c, double x,
    ...
    if (x < -0.5) {
        if (b > a)
            y = pow(s, -a) * ncephes_hys2f1(a, c - b, c, -x / s, &err);

        else
            y = pow(s, -b) * ncephes_hys2f1(c - a, b, c, -x / s, &err);
    ...
    if (x > 0.9) {
        ...
        {
            /* Try the power series first */
            y = ncephes_hys2f1(a, b, c, x, &err);
            if (err < ETHRESH)
                goto done;
            /* If power series fails, then apply AMS55 #15.3.6 */
            q = ncephes_hys2f1(a, b, 1.0 - d, s, &err);
            q *= ncephes_gamma(d) /
                 (ncephes_gamma(c - a) * ncephes_gamma(c - b));
            r = pow(s, d) * ncephes_hys2f1(c - a, c - b, d + 1.0, s, &err1);
            r *= ncephes_gamma(-d) / (ncephes_gamma(a) * ncephes_gamma(b));
            y = q + r;
            ...
            /* sum for t = 0 */
            y = ncephes_psi(1.0) + ncephes_psi(1.0 + e) - ncephes_psi(a + d1) -
                ncephes_psi(b + d1) - ax;
            y /= ncephes_gamma(e + 1.0);

            p = (a + d1) * (b + d1) * s /
                ncephes_gamma(e + 2.0); /* Poch for t=1 */
            t = 1.0;
            do {
                r = ncephes_psi(1.0 + t) + ncephes_psi(1.0 + t + e) -
                    ncephes_psi(a + t + d1) - ncephes_psi(b + t + d1) - ax;
                q = p * r;
                y += q;
                p *= s * (a + t + d1) / (t + 1.0);
                p *= (b + t + d1) / (t + 1.0 + e);
            ...
            if (id == 0.0) {
                y *= ncephes_gamma(c) / (ncephes_gamma(a) * ncephes_gamma(b));
                goto psidon;
            }
        ...
        nosum:
            p = ncephes_gamma(c);
            y1 *= ncephes_gamma(e) * p /
                  (ncephes_gamma(a + d1) * ncephes_gamma(b + d1));

            y *= p / (ncephes_gamma(a + d2) * ncephes_gamma(b + d2));
            if ((aid & 1) != 0)
                y = -y;
