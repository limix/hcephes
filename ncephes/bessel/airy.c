...
int ncephes_airy(double x, double *ai, double *aip, double *bi, double *bip) {
    ...
    if (x < -2.09) {
        ...
        z = 1.0 / zeta;
        zz = z * z;
        uf = 1.0 + zz * ncephes_polevl(zz, AFN, 8) / ncephes_p1evl(zz, AFD, 9);
        ug = z * ncephes_polevl(zz, AGN, 10) / ncephes_p1evl(zz, AGD, 10);
        theta = zeta + 0.25 * NCEPHES_PI;
        f = sin(theta);
        g = cos(theta);
        *ai = k * (f * uf - g * ug);
        *bi = k * (g * uf + f * ug);
        uf =
            1.0 + zz * ncephes_polevl(zz, APFN, 8) / ncephes_p1evl(zz, APFD, 9);
        ug = z * ncephes_polevl(zz, APGN, 10) / ncephes_p1evl(zz, APGD, 10);
        k = sqpii * t;
        *aip = -k * (g * uf + f * ug);
        *bip = k * (f * uf - g * ug);
        return (0);
    ...
    if (x >= 2.09) /* cbrt(9) */
    {
        ...
        k = 2.0 * t * g;
        z = 1.0 / zeta;
        f = ncephes_polevl(z, AN, 7) / ncephes_polevl(z, AD, 7);
        *ai = sqpii * f / k;
        k = -0.5 * sqpii * t / g;
        f = ncephes_polevl(z, APN, 7) / ncephes_polevl(z, APD, 7);
        *aip = f * k;

        if (x > 8.3203353) /* zeta > 16 */
        {
            f = z * ncephes_polevl(z, BN16, 4) / ncephes_p1evl(z, BD16, 5);
            k = sqpii * g;
            *bi = k * (1.0 + f) / t;
            f = z * ncephes_polevl(z, BPPN, 4) / ncephes_p1evl(z, BPPD, 5);
            *bip = k * t * (1.0 + f);
            return (0);
        }
    }
