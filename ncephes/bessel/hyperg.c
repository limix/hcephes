...
double ncephes_hyperg(double a, double b, double x) {
    ...
    if (fabs(temp) < 0.001 * fabs(a))
        return (exp(x) * ncephes_hyperg(temp, b, -x));

    psum = ncephes_hy1f1p(a, b, x, &pcanc);
    if (pcanc < 1.0e-15)
        goto done;

    /* try asymptotic series */

    asum = ncephes_hy1f1a(a, b, x, &acanc);
...
static double ncephes_hy1f1a(double a, double b, double x, double *err) {
    ...
    h1 = ncephes_hyp2f0(a, a - b + 1, -1.0 / x, 1, &err1);

    temp = exp(u) / ncephes_gamma(b - a);
    h1 *= temp;
    err1 *= temp;

    h2 = ncephes_hyp2f0(b - a, 1.0 - a, 1.0 / x, 2, &err2);

    if (a < 0)
        temp = exp(t) / ncephes_gamma(a);
    else
        temp = exp(t - ncephes_lgam(a));
