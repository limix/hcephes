...
double ncephes_smirnovi(int n, double p) {
    ...
    if (p <= 0.0 || p > 1.0) {
        ncephes_mtherr("smirnovi", NCEPHES_DOMAIN);
        return 0.0;
    }
    ...
    do {
        ...
        if (fabs(dpde) > 0.0)
            ...
            ncephes_mtherr("smirnovi", NCEPHES_UNDERFLOW);
            return 0.0;
        }
        e = e + t;
        if (e >= 1.0 || e <= 0.0) {
            ncephes_mtherr("smirnovi", NCEPHES_OVERFLOW);
            return 0.0;
        }
...
double ncephes_kolmogi(double p) {
    ...
    if (p <= 0.0 || p > 1.0) {
        ncephes_mtherr("kolmogi", NCEPHES_DOMAIN);
        return 0.0;
    }
    ...
    do {
        ...
        if (fabs(dpdy) > 0.0)
            ...
            ncephes_mtherr("kolmogi", NCEPHES_UNDERFLOW);
            return 0.0;
        }
        y = y + t;
